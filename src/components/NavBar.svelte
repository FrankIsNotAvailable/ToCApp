...
<div
    class="navBar sticky top-0 z-40 flex w-full shrink-0 items-center justify-between 
            ...
            sm:px-6 md:py-3"
>
    <!-- Brand Title (Adjusted font size so it won't push nav items) -->
    <div class="navBarTitle shrink-0 text-lg font-normal tracking-tight sm:text-2xl lg:text-[3rem]">
        Obscura+
    </div>

    <!-- Navigation Links / User Profile Area -->
    <AuthGuard requireAuth={true}>
        ...
        >
            ...
            <div class="relative h-7 w-7 shrink-0 sm:h-9 sm:w-9 md:h-11 md:w-11">
                ...
                />
                <button
                    class="profile-menu absolute -bottom-0.5 -right-0.5 flex h-3.5 w-3.5 cursor-pointer items-center 
                ...
                {#if showMenu}
                    <button
                        class="absolute right-0 top-full z-50 mt-2 flex cursor-pointer rounded-lg border 
                                border-gray-100 bg-white whitespace-nowrap shadow-[0_4px_12px_0_rgba(0,0,0,0.15)] 
                                hover:bg-gray-100"
                        onclick={handleLogout}
                    >
                        <div class="flex items-center gap-2 rounded-lg px-3 py-2 sm:px-4">
                            <img src={logout} alt="logout" class="h-4 w-4 sm:h-5 sm:w-5" />
                            <div class="text-xs font-medium text-gray-900 sm:text-sm">Log out</div>
                        </div>
                    </button>
                {/if}
            </div>
