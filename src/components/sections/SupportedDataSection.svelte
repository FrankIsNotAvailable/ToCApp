...
<div class="min-h-screen" id="supported-data">
	<div
		class="flex h-120 min-h-screen w-full flex-col items-center justify-center gap-10 px-5 py-12 sm:px-8 sm:py-16 md:px-12 lg:flex-row lg:justify-between lg:gap-16 lg:px-[clamp(4rem,15vw,20rem)] lg:py-12"
	>
		<div
		...
		<div class="h-120 max-h-200 w-full max-w-xl shrink-0 sm:h-136 lg:h-full lg:w-[40%] lg:max-w-none">
			<div
				class="flex h-full w-full flex-col justify-between overflow-hidden rounded-[17px] border border-white/25 bg-white/50 p-6 shadow-[0px_4px_25px_0px_rgba(0,0,0,0.25)] sm:p-8"
			>
				<div class="relative min-h-0 w-full flex-1 overflow-hidden">
					{#key currentNumber}
						<div
							class="absolute inset-0 h-full w-full overflow-y-auto"
